...
# 启动 Registry 转发
alias start-registry='nohup minikube kubectl -- port-forward -n kube-system service/registry 5000:80 >/tmp/minikube-registry.log 2>&1 &'
# 停止 Registry 转发
alias stop-registry='pkill -f "port-forward -n kube-system service/registry 5000:80"'

# --- Kong 管理 ---
# 启动 Kong 转发
alias start-kong='nohup minikube kubectl -- port-forward -n kong service/kong-kong-proxy 8080:80 >/tmp/minikube-kong.log 2>&1 &'
# 停止 Kong 转发
